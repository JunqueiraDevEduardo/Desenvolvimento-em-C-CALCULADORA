...
/*
include dynamic linking function header
*/
#include<dlfcn.h>
/*
...
{	//add for dynamic
	...
	/*
		Carregar biblioteca com  dlopen()
		Obter apontador para símbolo com  dlsym()
		Usar objecto apontado (*...)()
		Libertar biblioteca com  dlclose()
	*/
	case '+':
	...
	case '*':
        handle = dlopen("liboperacoes.so",RTLD_LAZY);
	foper = dlsym(handle,"mult");
	res = (*foper)(param1,param2);
	dlclose(handle);
        break;
	case '/':
        handle = dlopen("liboperacoes.so",RTLD_LAZY);
	foper = dlsym(handle,"divi");
	res = (*foper)(param1,param2);
	dlclose(handle);
        break;
	case '^':
        handle = dlopen("liboperacoes.so",RTLD_LAZY);
	foper = dlsym(handle,"plot");
	res = (*foper)(param1,param2);
	dlclose(handle);
        break;
	default:
